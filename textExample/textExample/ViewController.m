...
@property (nonatomic,strong) NudeIn *textView6;
...
- (void)loadView {
    ...
    ((UIScrollView *)self.view).contentSize = CGSizeMake([UIScreen mainScreen].bounds.size.width, 1200);
...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.textView6];
    self.textView6.frame = CGRectMake(0, 500, [UIScreen mainScreen].bounds.size.width, 300);

    [self.view addSubview:self.textView4];
    self.textView4.frame = CGRectMake(20, 900, 200, 150);
    
    [self.view addSubview:self.textView5];
    self.textView5.frame = CGRectMake(20, 1100, 0, 0);
...
- (NudeIn *)textView6 {
    if (!_textView6) {
        _textView6 = [NudeIn asyncMake:^(NUDTextMaker *make) {
            make.allImage().size(120,120).ln(3).attach();
            make.imageTemplate(@"im1").size(100,100).attach();
            make.allText().font(20).color([UIColor redColor]).skew(0).kern(6).ln(3).attach();
            make.textTemplate(@"shadow").shadowOffset(-5,-3).shadowBlur(5).shadowColor([UIColor redColor]).attach();
        ...
        _textView6.backgroundColor = [UIColor greenColor];
        _textView6.scrollEnabled = YES;
        _textView6.delaysContentTouches = YES;
//        [_textView6 asyncRemake:^(NUDTextMaker *make) {
//
//        }];
        
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self.textView6 update:^(NUDTextUpdate *update) {
                update.comp(2).asImage.size(150,150).apply();
                update.comp(0).asText.font(16).apply();
            }];
        });
